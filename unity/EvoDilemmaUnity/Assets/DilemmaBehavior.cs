    ...
    public class DilemmaBehavior : MonoBehaviour
    {
        ...
        [SerializeField] private Button automaticProgressionButton;
        ...
        private Dictionary<string, float> previousGenerationFitness = new Dictionary<string, float>(); 
        ...
        private Text generationButtonText;
        private TMP_Text generationButtonTMPText;
        ...
        private void Start()
        {
            ...
            automaticProgressionButton.onClick.AddListener(setAutomaticProgression);
            generationButtonText = generationButton.GetComponentInChildren<Text>();
            if(generationButtonText == null){
                generationButtonTMPText = generationButton.GetComponentInChildren<TMP_Text>();
            }
        }
        void Update()
        {
        ...
        void setAutomaticProgression()
        {
            automaticProgression = !automaticProgression;
            if(automaticProgression == true){ 
                automaticProgressionButton.GetComponent<Image>().color = Color.green; 
                Debug.Log("Automatic progression on");
                autoProgressCoroutine = StartCoroutine(AutomaticGenerationProgression());
            }
            else if(automaticProgression == false){ 
                automaticProgressionButton.GetComponent<Image>().color = Color.white; 
                Debug.Log("Automatic progression off");
        ...
        IEnumerator ProgressGeneration()
{
    ...
    using (UnityWebRequest fetchGeneration = UnityWebRequest.Get("http://localhost:8080/play_dilemma"))
    {
        ...
        if(fetchGeneration.result != UnityWebRequest.Result.Success){
            ...
            if(sortedTopFitness != null && sortedTopFitness.Count > 0){
                previousGenerationFitness = new Dictionary<string, float>(sortedTopFitness);
            }
            ...
            // CRITICAL FIX: Ensure agents list is properly populated after recreation
            if(CurrentGeneration > 0 && agents.Count == 0) {
                agents.AddRange(FindObjectsOfType<AgentScript>().Select(a => a.gameObject));
                Debug.Log($"Repopulated agents list with {agents.Count} agents after recreation");
            }
            ...
            CurrentGeneration++;
            if(generationButtonText != null){
                generationButtonText.text = $"Progress Generation: {CurrentGeneration}";
            }
            else if(generationButtonTMPText != null){
                generationButtonTMPText.text = $"Progress Generation: {CurrentGeneration}";
            }
            else{
                Debug.LogWarning("No Text or TMP_Text component found on generation button? "); // This shouldn't happen in any scenario
            }
        }
    }
}
        ...
        void PruneGeneration()
        {
            if(CurrentGeneration >= 1){
                Dictionary<string, float> fitnessToUse = previousGenerationFitness.Count > 0 ? previousGenerationFitness : sortedTopFitness;
                 if(fitnessToUse.Count == 0){
                    Debug.LogWarning("No fitness data available for pruning?");
                    return;
                }
                var agentIDsToKeep = new HashSet<string>(fitnessToUse.Keys);
                ...
                var survivingAgentIDs = new HashSet<string>(agentIDsToKeep);
                agentBehaviorList = agentBehaviorList.Where(entry => survivingAgentIDs.Contains(entry.Item1)).ToList();
                AgentScript.agentBehaviorList = agentBehaviorList;
        ...
        private IEnumerator AutomaticGenerationProgression()
        {
            while(automaticProgression)
            {
